...
def ensure_schema(schema_path: str | os.PathLike[str] = DEFAULT_SCHEMA_PATH) -> None:
    ...
    with connect(readonly=False) as conn:
        with conn.cursor() as cur:
            ...
            # Preferred icon storage: base64 text (plays nicely with drivers/UI)
            cur.execute(
                """
                ALTER TABLE scan_events
                ADD COLUMN IF NOT EXISTS app_icon_b64 TEXT
                """
            )
            cur.execute(
                """
                ALTER TABLE scan_events
                ADD COLUMN IF NOT EXISTS app_icon_b64_mime TEXT
                """
            )
...
def insert_scan_event(
    *,
    ...
    app_icon_b64: str | None = None,
    app_icon_b64_mime: str | None = None,
    score: int | None,
    risk_level: str | None,
